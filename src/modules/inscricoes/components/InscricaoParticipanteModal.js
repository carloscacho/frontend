...
export default function InscricaoParticipanteModal({ refModal, activityId, onSuccess }) {
    ...
    const submitEmails = async (emails) => {
        setIsLoading(true);
        try {
            const data = await inscricaoService.batchEnrollParticipants(activityId, emails);
            setResult(data);
            onSuccess?.();
        } catch (error) {
            const errData = error.response?.data || {};
            setErrors([errData.message || error.message || 'Erro de conexão ou ao inscrever participantes']);
        } finally {
            setIsLoading(false);
        }
    };
