...
export function useSchedule(atividades, evento) {
    ...
    const fetchRegistrations = useCallback(async () => {
        ...
        try {
            const data = await registrationService.fetchParticipantActivities(usuario.participante[0].id_participante);
    ...
    const handleParticipar = async (atividade) => {
        ...
        try {
            if (!participanteId) {
                const partData = await registrationService.ensureParticipant(usuario);
            ...
            if (evento && evento.id_evento) {
                await registrationService.linkParticipantToEvent(evento.id_evento, participanteId);
            }
            ...
            for (const session of atividade.data_atividade) {
                ...
                try {
                    const res = await registrationService.manageActivityRegistration(action, session, participanteId);
